...
void mode2()
{
 uint8_t  i, x, L, *data, nBins, binNum, weighting, c;
  ...
  matrix.fillRect(0, 0, 8, 3, matrix.Color(255,0,0));    // Upper section
  matrix.fillRect(0, 3, 8, 2, matrix.Color(255, 255, 0)); // Mid
  matrix.fillRect(0, 5, 8, 3, matrix.Color(0,255,0));  // Lower section
  ...
  for(x=0; x<8; x++) {
    ...
    if(peak[x] <= 0) { // Empty column?
      matrix.drawLine(x, 0, x, 7, matrix.Color(0,0,0));
      continue;
    } else if(c < 8) { // Partial column?
		matrix.drawLine(x, 0, x, 7 - c, matrix.Color(0, 0, 0));
    }
